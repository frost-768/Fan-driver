...
char *char_alt(char **input, const char *items, const char invert) {
	if (! **input) return NULL;//inoutΪ���򷵻�null
	while (*items)//items��Ϊ��
...
int *parse_int(char **input) {//���ձ��ʽ  ����������ֵ
	char *end = *input;
	int *rv = NULL;
	long int l;

	l = strtol(*input, &end, 0);//ת��Ϊ10����
...
char *parse_keyword(char **input, const char *keyword) {
	...
	if (!strncasecmp(*input, keyword, l)) {//strncasecmp()����Ƚϲ���s1��s2�ַ�ǰn���ַ�Ƚ�ʱ���Զ����Դ�Сд�Ĳ��죬��ͬ����0
		ret = *input;
		*input += l;
	}
	return ret;
