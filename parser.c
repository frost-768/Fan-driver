...
char *char_alt(char **input, const char *items, const char invert) {
	if (! **input) return NULL;//inout为空则返回null
	while (*items)//items不为空
...
int *parse_int(char **input) {//接收表达式  返回整形数值
	char *end = *input;
	int *rv = NULL;
	long int l;

	l = strtol(*input, &end, 0);//转换为10进制
...
char *parse_keyword(char **input, const char *keyword) {
	...
	if (!strncasecmp(*input, keyword, l)) {//strncasecmp()用来比较参数s1和s2字符串前n个字符，比较时会自动忽略大小写的差异，相同返回0
		ret = *input;
		*input += l;
	}
	return ret;
...
void skip_space(char **input) {
	char *tmp = char_cat(input, space, 0);//const char space[] = " \t\n\r\f";
	free(tmp);
}

char *parse_comment(char **input) {
	skip_space(input);
	if (!char_alt(input, comment, 0)) return NULL;//const char comment[] = "#";
	char *tmp = char_cat(input, newline, 1);//const char newline[] = "\n\r";
	skip_space(input);
	return tmp;
}
