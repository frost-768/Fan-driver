 ...
 *******************************************************************/
int get_temp_ibm() {//这个函数读取温度 返回最高的温度
...
void setfan_ibm() {
	...
	if (unlikely((ibm_fan = open(IBM_FAN, O_RDWR, O_TRUNC)) < 0)) {//打开文件出错则返回错误信息
		showerr(IBM_FAN);
		errcnt++;
	}
	else {
...
int init_fan_ibm() {
	...
	fprintf(ibm_fan, "watchdog %d\n", watchdog_timeout);//watchdog_timeout何用？
	fclose(ibm_fan);
	free(line);
	return 0;
}
